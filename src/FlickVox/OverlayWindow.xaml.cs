...
public partial class OverlayWindow : Window
{
 ...
 void InputChanged(object s,TextChangedEventArgs e)=>Placeholder.Visibility=string.IsNullOrEmpty(Input.Text)?Visibility.Visible:Visibility.Collapsed;
 async void KeyDownHandler(object s,System.Windows.Input.KeyEventArgs e){if(e.Key==Key.Enter&&!Keyboard.Modifiers.HasFlag(ModifierKeys.Shift)){e.Handled=true;var text=Input.Text;if(!string.IsNullOrWhiteSpace(text)){_items.Remove(text);_items.Insert(0,text);if(_settings.Current.HideOverlayAfterSpeaking){Save();Hide();}else Input.Clear();await _speech.SpeakAsync(text);}}else if(e.Key==Key.Escape){_speech.Stop();Save();Hide();}else if((e.Key is Key.Up or Key.Down)&&_items.Count>0){_cursor=Math.Clamp(_cursor+(e.Key==Key.Up?1:-1),0,_items.Count-1);Input.Text=_items[_cursor];Input.CaretIndex=Input.Text.Length;e.Handled=true;}}
